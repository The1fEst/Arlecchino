...
public sealed class Repaint : IDisposable
{
    ...
    public bool IsRequested => Volatile.Read(ref _requests) == 1;

    /// <summary>Asks for a frame. Safe to call from any thread and cheap to call repeatedly.</summary>
    public void Request() => Interlocked.Exchange(ref _requests, 1);
    ...
    public bool TakeRequested() => Interlocked.Exchange(ref _requests, 0) == 1;
