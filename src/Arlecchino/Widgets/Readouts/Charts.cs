...
/// <summary>
/// A series of numbers as one row of blocks, tallest for the largest of them, with no axis, scale or grid.
/// The newest value is the rightmost, and only as many as fit the row are drawn.
/// </summary>
public sealed class Sparkline : IArlecchinoWidget
{
    ...
    /// <summary>
    /// The numbers to draw, the oldest first. Nothing is copied, so a ring buffer the application appends to
    /// between frames is the right thing to hand over.
    /// </summary>
    public IReadOnlyList<decimal> Values { get; set; } = [];

    /// <summary>
    /// The value the lowest block stands for, which is the smallest of the drawn values when left alone.
    /// Pinning it keeps the line still where the numbers barely change.
    /// </summary>
    ...
    /// <summary>
    /// Draws the line across the first row of the region, leaving room for the caption when there is one. A
    /// series with no spread at all draws as the lowest block rather than as a full row.
    /// </summary>
...
/// <summary>
/// A series drawn as a filled area over as many rows as it is given, where <see cref="Sparkline"/> fits one.
/// A cell carries two samples side by side, so it holds twice the history a row of blocks would.
/// </summary>
public sealed class AreaChart : IArlecchinoWidget
{
    ...
    /// <summary>
    /// What to draw with, which is the application's own <see cref="Glyphs.Graph"/> when left alone. One
    /// chart can differ without every other one being told.
    /// </summary>
    public GraphSymbols? Symbols { get; set; }

    /// <summary>
    /// Where the color changes as the fill climbs, in the same units as the values and in ascending order. A
...
/// behind. Bars are measured against the largest item unless told otherwise.
/// </summary>
...
public sealed class BarChart<T> : IArlecchinoWidget
{
    ...
    /// <summary>
    /// Draws a bar for every item that fits and returns the rows below them. Items past the bottom of the
    /// region are not drawn, since the chart does not scroll.
    /// </summary>
    /// <param name="region">Where to draw.</param>
...
/// <summary>
/// Where a band of a <see cref="Gauge"/> starts and how it is drawn. A band runs from <see cref="From"/> up
/// to the start of the next one, so bands are given in ascending order.
/// </summary>
...
/// <summary>
/// One value against a range, drawn as a bar whose color changes as it crosses the bands it was given. Each
/// part of the fill keeps the color of its own band.
/// </summary>
