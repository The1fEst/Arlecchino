...
public sealed class ViewNavigationGenerator : IIncrementalGenerator
{
    ...
    private static string Generate(IReadOnlyList<ViewModel> views, string viewNamespace)
    {
        return $$"""
            ...
            using Arlecchino.Navigation;{{Imports(views, viewNamespace)}}

            namespace {{viewNamespace}};

            public static class ViewKind
            {
                public static ViewRoute None => ViewRoute.None;{{Routes(views)}}
            }
            ...
            public static class GeneratedViewRegistration
            {
                ...
                {
                    return builder.AddViewFactory<GeneratedViewFactory>();
                }
            }
    ...
    /// </summary>
    /// <param name="views">The views being generated.</param>
    /// <param name="viewNamespace">Where the generated file itself lives.</param>
    /// <returns>The lines, or an empty string.</returns>
    private static string Imports(IReadOnlyList<ViewModel> views, string viewNamespace)
    {
        var names = views
    ...
    private static string Routes(IReadOnlyList<ViewModel> views) =>
        Following(views.Select(static view =>
            $"""    public static readonly ViewRoute {view.RouteName} = new ViewRoute("{view.RouteName}");"""));

    private static string Lookup(IReadOnlyList<ViewModel> views)
    {
        if (views.Count == 0)
        {
    ...
    /// <summary>
    /// Joins lines so that they follow whatever is already written, each led by a newline rather than
    /// separated by one. An empty sequence then adds nothing at all.
    /// </summary>
    /// <param name="lines">The lines.</param>
    /// <returns>The lines, or an empty string.</returns>
    private static string Following(IEnumerable<string> lines) =>
        string.Concat(lines.Select(static line => "\n" + line));
