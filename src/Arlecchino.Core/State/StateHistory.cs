...
/// <summary>
/// Undo and redo over every atom that records history. It collects from the moment it exists, so the
...
/// </summary>
public sealed class StateHistory : IDisposable
{
    private readonly LinkedList<List<IStateEdit>> _undoable = new();
    private readonly Stack<List<IStateEdit>> _redoable = new();

    private int _capacity = 200;
    private List<IStateEdit>? _group;
    private bool _isReplaying;

    /// <summary>Starts collecting edits.</summary>
    public StateHistory()
    {
        StateChanges.Recorded += Record;
    }

    /// <summary>
    /// How many steps to keep. Lowering it drops the oldest straight away; anything below one step is
    /// treated as one, since a history that remembers nothing is what <c>RecordsHistory</c> is for.
    /// </summary>
    public int Capacity
    {
        get => _capacity;
    ...
    public void Dispose() => StateChanges.Recorded -= Record;

    private void Record(IStateEdit edit)
    {
        if (_isReplaying)
        {
            return;
    ...
    private void Remember(List<IStateEdit> step)
    {
        _undoable.AddLast(step);
        _redoable.Clear();
        DropOldest();
    ...
    private sealed class GroupScope : IDisposable
    {
        ...
        public GroupScope(StateHistory history)
        {
            _history = history;
        }
